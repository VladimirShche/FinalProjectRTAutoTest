from settings import valid_email, sql_injection, random_init
...
class AuthPage(BasePage):
    #EX-001 метод проверки перехода на страницу авторизации на сайте Ростелеком
    ...
    #EX-004 метод проверки ссылки на форму восстановления пароля
    ...
    #EX-005 метод проверки типа аутентификации по умолчанию
    ...
    #EX-006 метод проверки ссылки на форму регистрации
    ...
    #EX-007 метод проверки автоматического изменения типа аутентификации
    ...
    #EX-008 метод проверки ссылки под кнопкой "Войти" на страницу с пользовательским соглашением
    ...
    #EX-009 метод проверки ссылки на страницу авторизации с помощью социальной сети "Одноклассники"
    def link_to_social_ok(self):
        self.find_element(AuthPageLocators.AUTH_SOCIAL_OK_LINK).click()
        assert "https://connect.ok.ru/oauth/authorize" in self.browser.current_url, "url do not match"

    #EX-010 метод проверка работы ссылки с пользовательским соглашением в "подвале" сайта
    ...
    #EX-011 метод проверки авторизации с незаполненными полями
    ...
    #EX-012 метод проверки текстового поля на SQL-инъекции
    def sql_injection_in_a_text_field(self):
        self.find_element(AuthPageLocators.AUTH_USERNAME_INPUT).send_keys(sql_injection())
        self.find_element(AuthPageLocators.AUTH_PASSWORD_INPUT).send_keys(random_init())
        self.find_element(AuthPageLocators.AUTH_ENTER_BUTTON).click()
